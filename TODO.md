...
- [ ] **Memory** — `.oneloop/memory.md`, a single markdown file the agent reads and writes. Loaded into system prompt at startup (alongside AGENTS.md). During compaction, the model extracts worth-keeping facts and appends them. Capped at ~200 lines — oldest entries trimmed. No vector DB, no embeddings. The model decides what to remember.
